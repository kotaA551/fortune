...
const COLORS = {
  ...
  heading: "#16a34a", // emerald-600（16P風の落ち着いたグリーン）
...
function footerPageNumber(doc: PDFKit.PDFDocument, current: number, total: number) {
  ...
  doc.fillColor(COLORS.meta).fontSize(10).text(label, 0, height - 32, {
    width,
    align: "center"
  });
}
...
function drawCoverPage(doc: PDFKit.PDFDocument, order: Order) {
  ...
  doc.fillColor(COLORS.title).fontSize(42).text(title, 0, cardY + 26, {
    width,
    align: "center"
  });
  doc.fillColor(COLORS.sub).fontSize(18).text(subtitle, 0, cardY + 26 + 56, {
    width,
    align: "center"
  });
  ...
  doc.fillColor(COLORS.meta).fontSize(11).text(meta, 0, cardY + cardH + 24, {
    width,
    align: "center"
  });
}
...
function drawSectionPage(doc: PDFKit.PDFDocument, title: string, body: string, isLast: boolean) {
  ...
  // 見出し
  doc.fillColor(COLORS.heading).fontSize(22).text(title, PAGE_MARGIN, PAGE_MARGIN, {
  ...
  doc
    ...
    .lineTo(width - PAGE_MARGIN, PAGE_MARGIN + 32)
    .stroke();
  ...
  doc.fillColor(COLORS.body).fontSize(12).lineGap(6).text(body, colX, topY, {
    width: colW,
    align: "left",
    paragraphGap: 10
  });
  ...
  if (isLast) {
    ...
    doc.fillColor(COLORS.disclaimer).fontSize(10).text(
      ...
      PAGE_MARGIN,
      disclaimerY,
      { width: width - PAGE_MARGIN * 2, align: "left" }
    );
  }
}
...
async function composeAllPages(doc: PDFKit.PDFDocument, order: Order) {
  ...
  // すべて描いた後にフッター（総ページ数つき）を書き込む
  const range = doc.bufferedPageRange();
  const total = range.count;
  for (let i = 0; i < total; i++) {
    doc.switchToPage(i);
    footerPageNumber(doc, i + 1, total);
  }
}

// ---- Public APIs -------------------------------------------------------
export async function generatePdfBuffer(order: Order): Promise<Buffer> {
  // bufferPages: true → 全ページをメモリに保持して後からフッターを描ける
